...
TEST(Syntax_Analyze_Parse, SY_RIGHT_PARENTHESIS_single_token)
{
	t_token **tokens = ms_lexical_analyze(")");
	t_syntax_node *node = ms_parse_right_parenthesis(tokens[0], 0);
	ASSERT_NE(node, nullptr);
	EXPECT_EQ(node->type, SY_RIGHT_PARENTHESIS);
