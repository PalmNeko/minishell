...
TEST(Syntax_Analyze_Parse, SY_LEFT_PARENTHESIS_single_token)
{
	t_token **tokens = ms_lexical_analyze("(");
	t_syntax_node *node = ms_parse_left_parenthesis(tokens[0], 0);
	ASSERT_NE(node, nullptr);
	EXPECT_EQ(node->type, SY_LEFT_PARENTHESIS);
