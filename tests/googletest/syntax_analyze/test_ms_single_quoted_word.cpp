...
TEST(Syntax_Analyze_Parse, SY_SINGLE_QUOTED_WORD_SUCESS)
{
	t_token **tokens = ms_lexical_analyze("\'Hello\'");
	t_syntax_node *node = ms_parse_single_quoted_word(tokens, 0);
	ASSERT_NE(node, nullptr);
	EXPECT_EQ(node->type, SY_SINGLE_QUOTED_WORD);
	EXPECT_EQ(node->start_pos, 0);
	EXPECT_EQ(node->end_pos, 3);
	for(int i = 0; node->children[i]; i++)
	{
		if(i == 0 || i == node->end_pos - node->start_pos - 1)
			EXPECT_EQ(node->children[i]->type, SY_SINGLE_QUOTE);
		else
	...
	node = ms_parse_single_quoted_word(tokens, 0);
	ASSERT_NE(node, nullptr);
	EXPECT_EQ(node->type, SY_SINGLE_QUOTED_WORD);
	EXPECT_EQ(node->start_pos, 0);
...
TEST(Syntax_Analyze_Parse, SY_SINGLE_QUOTED_WORD_FAIL)
{
	t_token **tokens = ms_lexical_analyze("\"Hello");
	t_syntax_node *node = ms_parse_single_quoted_word(tokens, 0);
	ASSERT_NE(node, nullptr);
	...
	EXPECT_EQ(node->start_pos, 0);
	EXPECT_EQ(node->end_pos, 3);
	EXPECT_EQ(node->children[0]->type, SY_SINGLE_QUOTE);
