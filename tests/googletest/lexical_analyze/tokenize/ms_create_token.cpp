...
TEST(TokenTest, AllTokenTypes)
{
	...
	const struct TestCase test_cases[] = {{(t_token_type)TK_DECLINED, "???", 0,
		2, "Declined token"},
											{(t_token_type)TK_IDENTIFY,
												"identifier123", 0, 11,
												"Identifier token"},
											{(t_token_type)TK_WORD, "hello", 0,
												4, "Word token"},
											{(t_token_type)TK_BLANK, " \t", 0,
												1, "Blank token"},
											{(t_token_type)TK_NEWLINE, "\n", 0,
												0, "Newline token"},
											{(t_token_type)TK_VARIABLE, "$PATH",
												0, 4, "Variable token"},
											{(t_token_type)TK_PIPE, "|", 0, 0,
												"Pipe token"},
											{(t_token_type)TK_EQUALS, "=", 0, 0,
												"Equals token"},
											{(t_token_type)TK_SINGLE_QUOTE, "'",
												0, 0, "Single quote token"},
											{(t_token_type)TK_DOUBLE_QUOTE,
												"\"", 0, 0,
												"Double quote token"},
											{(t_token_type)TK_REDIRECTION, "<",
												0, 0,
												"Redirection input token"},
											{(t_token_type)TK_REDIRECTION, "<<",
												0, 1,
												"Redirection heredoc token"},
											{(t_token_type)TK_REDIRECTION, ">",
												0, 0,
												"Redirection output token"},
											{(t_token_type)TK_REDIRECTION, ">>",
												0, 1,
												"Redirection append token"},
											{(t_token_type)TK_LEFT_PARENTHESIS,
												"(", 0, 0,
												"Left parenthesis token"},
											{(t_token_type)TK_RIGHT_PARENTHESIS,
												")", 0, 0,
												"Right parenthesis token"},
											{(t_token_type)TK_LIST, "&&", 0, 1,
												"List AND token"},
											{(t_token_type)TK_LIST, "||", 0, 1,
												"List OR token"}};
